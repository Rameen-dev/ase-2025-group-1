// This component fetches the donor’s sustainability impact 
// from the backend and displays it as cards on the donor dashboard (Home page).
...
// This component will display the sustainability impact cards, on the donor dashboard (Home page)
export default function DonorImpactCards() {
  // We track whether we are still loading data from the API
  const [loading, setLoading] = useState(true);
  // We store any error message if the request fails.
  const [error, setError] = useState<string | null>(null);

  // Here we store any data returned from the /api/impact/donor, such as the total Items.
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const res = await fetch("/api/impact/donor", {
          method: "GET",
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
      ...
      {/* A small disclaimer explaining the numbers are estimates to the donors */}
